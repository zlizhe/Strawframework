...
//        $this->_getConnect();
    ...
    public function getAll() : array {
        ...
        if ($this->_modelData['cacheKey']) {
            //有缓存 数据优先使用
            $cacheRes = json_decode(\strawframework\base\Cache::get($this->_modelData['cacheKey']), TRUE);
            if ($cacheRes) {
                //取到数据 清空条件
                $this->_modelData = [];
        ...
        if ($this->_modelData['cacheKey']) {
            \strawframework\base\Cache::set($this->_modelData['cacheKey'], json_encode($result), $this->_modelData['exp']);
        }
