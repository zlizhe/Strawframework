...
final class Valid {
     ...
     * @return bool
     */
    public static function fromMobile(): bool {
     ...
     * @return bool
     */
    public static function domain(string $url, $domain): bool {
        if (is_array($domain)) {
            $domainArr = [];
...
//    print_r($matches);die;
     ...
     * @return null|string
     */
    public static function ref(string $siteDomain, string $defaultDomain = '') : ? string {

        $ref = urldecode($_GET['ref'] ?: $_POST['ref']);

        if ($ref) {
            if (TRUE == self::domain($ref, $siteDomain)) {
                return $ref;
            }
        }
